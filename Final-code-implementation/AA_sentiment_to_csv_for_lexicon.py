...
This is the conversation code from conversatoins.py and comments on the code can
 be found there. The code is looped for American Airlines to create a CSV file of 
 sentiment scores and tweet_text for American Airlines, which we in turn use for the creation of the lexicons.
'''
counterair = 0
...
# Default for AA
    ...
    # Add all text of tweets to a American Airlines and its tweet_id to seperate
    # lists and create a dataframe which will store the sentiment scores and tweet_text for American Airlines.
    df_sentiment_text= pd.DataFrame()
    df_sentiment_text['text'] = 0
    df_sentiment_text['sentiment'] = 0
    tweet_ids_lst = []
    tweet_text_lst = []
    ...
    for key in Conversation.tweets.keys():
        if Conversation.tweets[key].user != user_id:
            ...
            counter += 1
    df_sentiment_text['text'] = tweet_text_lst
    df_sentiment_text['sentiment'] = tweet_sentiment_score
    # Add the sentiment scores and text of American Airline to the dataframe.

df_sentiment_text.to_csv('sentiment_AA.csv')
